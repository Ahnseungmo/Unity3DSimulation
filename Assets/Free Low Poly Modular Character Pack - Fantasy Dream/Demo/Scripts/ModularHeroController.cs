...
namespace GanzSe
{
    public class ModularHeroController : MonoBehaviour
    {
        ...
        public void SetAromorPart(int num)
        {
            if (armorPartsRoot == null) return;
            foreach (Transform category in armorPartsRoot)
            {
                SetSelectActiveChild(category,num);
            }
        }
        ...
        private void SetSelectActiveChild(Transform category,int num)
        {
            if (category.childCount == 0) return;
            if (category.childCount < num) return;

            foreach (Transform child in category)
            {
                child.gameObject.SetActive(false);
            }
            category.GetChild(num).gameObject.SetActive(true);
        }
    }
    ...
    public class ModularCharacterEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            //            string option;
            option = EditorGUILayout.IntField("ArmorSet Number", option);

            if (GUILayout.Button("Set ArmorSet"))
            {
                controller.SetAromorPart(option);
            }
        }
    }
#endif
